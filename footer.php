...
.footer-brand-compact{display:flex;align-items:center;gap:9px;margin-bottom:7px}.footer-site-logo{display:flex;align-items:center;justify-content:center;flex:none;width:48px;height:48px}.footer-site-logo img{width:100%;height:100%;object-fit:contain;display:block}.footer-brand-compact h3{font-size:11px;margin:0 0 2px}.footer-brand-compact span{font-size:8px;color:#d7e3f2}.footer-about>p{font-size:8px;line-height:1.8;max-width:260px}
...
<div class="footer-grid footer-grid-compact">
 <div class="footer-col footer-about" id="about"><div class="footer-brand footer-brand-compact">
 <?php
 if (has_custom_logo()) {
     $custom_logo_id = get_theme_mod('custom_logo');
     $logo_url = wp_get_attachment_image_url($custom_logo_id, 'full');
     if ($logo_url) {
         echo '<a class="footer-site-logo" href="' . esc_url(home_url('/')) . '" rel="home" aria-label="' . esc_attr(get_bloginfo('name')) . '"><img src="' . esc_url($logo_url) . '" alt="' . esc_attr(get_bloginfo('name')) . '"></a>';
     } else {
         echo '<div class="footer-site-logo">' . ramser_energy_logo() . '</div>';
     }
 } else {
     echo '<div class="footer-site-logo">' . ramser_energy_logo() . '</div>';
 }
 ?>
 <div><h3><?php echo esc_html(ramser_energy_get('brand_title')); ?></h3><span><?php echo esc_html(ramser_energy_get('brand_subtitle')); ?></span></div></div><p><?php echo nl2br(esc_html(ramser_energy_get('footer_about'))); ?></p></div>
